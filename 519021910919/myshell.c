...
int main(int argc, char **argv)
{
    ...
    while (!feof(instream))
    {
        ...
        // TODO
        if(instream == stdin) {
            printf("%s%s", getcwdstr(cwdbuf, sizeof(cwdbuf)),prompt); //get the cwd
            fflush(stdout);
        }
        ...
        if (fgets(linebuf, MAX_BUFFER, instream))
        {
            ...
            *arg++ = strtok(linebuf, SEPARATORS); // tokenize input
            ...
            if (args[0])
            {
                ...
                {
                    // TODO
                    if (!args[1]) {            // no directory argument, print cwd
                ...
                {
                    // TODO
                    getpass("Press Enter to continue ..."); //print the message
                    execargs = FALSE;           // no need for further exec
                }
                // "quit" command
                ...
                // TODO
                 if (execargs)
                    execute(args, status); //fork call the system call
            }
        }
    }
    return 0;
...
{
    // TODO
    if(!getcwd(buffer, size)) {  // get current working directory
